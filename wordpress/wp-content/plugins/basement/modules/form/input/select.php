...
class Basement_Form_Input_Select extends Basement_Form_Input {
	...
	public function create() {
		...
		if ( isset($this->config['dom_change']) !== 'remove_span_before_select' ) {
			$label->appendChild( $this->dom->createElement( 'span', isset($label_text) ? $label_text : '' ) );
		}



		/*if($this->config['name'] !== 'col_style_border' && $this->config['name'] !== 'col_unit_border' && $this->config['name'] !== 'row_style_border' && $this->config['name'] !== 'row_unit_border') {
		...
		foreach ( $values as $value => $option_text) {
			$option = $this->dom->createElement( 'option', esc_html($option_text) );
