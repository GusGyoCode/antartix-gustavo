...
$caption = $img_link_large = $link = $img_link_target = $alignment = $height = $onhover = $el_class = $el_id = $label = $css_animation = $style = $external_style = $border_color = $css = '';
...
if(is_numeric($height)) {
	$wrap_style[] = "height:{$height}px";
	$wrap_style[] = "overflow:hidden";
...
if(!empty($wrap_style)) {
	$wrap_style = 'style="' . implode(';',$wrap_style) . '"';
} else {
	$wrap_style = '';
}
...
if ( $link ) {
	...
	//$a_attrs['class'] = 'vc_img_link_wrap';

	#$html = '<a ' . vc_stringify_attributes( $a_attrs ) . ' ><div '.$wrap_style.' class="' . $wrapperClass . '">' . $img['thumbnail'] . '</div></a>';
	$html = '<a '.$wrap_style.' ' . vc_stringify_attributes( $a_attrs ) . ' class="vc_img_link_wrap ' . $wrapperClass . '">' . $img['thumbnail'] . '</a>';
} else {
	$html = '<div '.$wrap_style.' class="' . $wrapperClass . '">' . $img['thumbnail'] . '</div>';
}
...
if(!empty($onhover) && ($onclick !== 'zoom' && $onclick !== 'link_image' || empty($onclick))) {
	$class_to_filter .= ' vc_single_image_' . $onhover . ' ';
}
...
if(!empty($label)) {
	$label = '<div class="vc_image_label">' . esc_html($label) . '</div>';
}
