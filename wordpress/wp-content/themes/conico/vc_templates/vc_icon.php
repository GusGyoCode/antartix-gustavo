 ...
 */
$type = $icon_fontawesome = $display = $vertical_align = $icon_openiconic = $icon_typicons = $icon_entypo = $icon_linecons = $color = $custom_color = $background_style = $background_color = $custom_background_color = $size = $align = $el_class = $el_id = $link = $css_animation = $css = $rel = '';

$atts = vc_map_get_attributes( $this->getShortcode(), $atts );
extract( $atts );

if(!empty($display)) {
	$display = 'vc_icon_display_' . $display;
}

$class_to_filter = '';
$class_to_filter .= vc_shortcode_custom_css_class( $css, ' ' ) . $this->getExtraClass( $el_class ) . $this->getCSSAnimation( $css_animation );
$css_class = apply_filters( VC_SHORTCODE_CUSTOM_CSS_FILTER_TAG, $display. ' ' . $class_to_filter, $this->settings['base'], $atts );
...
if(!empty($vertical_align) && $display === 'vc_icon_display_inline') {
	$wrapper_attributes[] = 'style="vertical-align:'.esc_attr($vertical_align).';"';
}
