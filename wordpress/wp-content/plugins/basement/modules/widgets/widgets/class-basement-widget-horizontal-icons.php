<?php
class Basement_Horizontal_Icons_Widget extends WP_Widget {

	public function __construct() {
		$widget_ops = array(
				'classname'   => 'widget_icon_list',
				'description' => __( 'A icon list.', BASEMENT_TEXTDOMAIN ),
				'customize_selective_refresh' => true
		);
		parent::__construct(
			'icon',
			__('Horizontal Icon List', BASEMENT_TEXTDOMAIN),
			$widget_ops
		);
	...
	public function widget( $args, $instance ) {
		extract($args);
		$title = $before_title . apply_filters( 'widget_title', empty( $instance['title'] ) ? __( 'Horizontal icons', BASEMENT_TEXTDOMAIN ) : $instance['title'], $instance, $this->id_base ) . $after_title;
		...
		$size = empty($instance['size']) ? '24' : $instance['size'] ;
		...
		if ($type == "ordered") { echo "<ol ";} else { echo("<ul "); } ?> class=" text-<?php echo $align; ?>">

		<?php $i = 0; foreach ($items as $num => $item) {
			if ( ! empty( $item ) && $i !== 0 ) {
				if ( empty( $item_links[ $num ] ) ) :
					$full_size = "font-size:{$size}px;width:{$size}px;height:{$size}px;line-height:{$size}px;";
					echo( "<li class='" . esc_attr( $item_classes[ $num ] ) . "'><i style='".$full_size."' class='". esc_attr( $item )."'></i></li>" );
				else :
					$full_size = "font-size:{$size}px;width:{$size}px;height:{$size}px;line-height:{$size}px;";
					if ( $item_targets[ $num ] ) :
						echo( "<li class='" . esc_attr( $item_classes[ $num ] ) . "'><a href='" . $item_links[ $num ] . "' target='_blank'><i style='".$full_size."' class='".esc_attr($item)."'></i></a></li>" );
					else :
						echo( "<li class='" . esc_attr( $item_classes[ $num ] ) . "'><a href='" .  $item_links[ $num ] . "'><i style='".$full_size."' class='".esc_attr($item)."'></i></a></li>" );
					endif;
				endif;
			}
			$i++;
	...
	public function form( $instance ) {
		...
		<div class="wrapper-widget">
			...
			<p><label for="<?php echo $this->get_field_id('size'); ?>"><?php _e('Icon size <small>(in px)</small>:', BASEMENT_TEXTDOMAIN); ?></label>
				<input class="widefat " id="<?php echo $this->get_field_id('size'); ?>" name="<?php echo $this->get_field_name('size'); ?>" type="number" value="<?php echo absint($size); ?>" /></p>
			...
			<div class="simple-link-list">
				<?php $i = 0; foreach ($items as $num => $item) {
					...
					<div id="<?php echo $this->get_field_id($num); ?>" class="list-item <?php if($i === 0) { echo 'template'; $i++; } ?>">
						<h5 class="moving-handle"><span class="number"><?php _e('Item:', BASEMENT_TEXTDOMAIN); ?></span> <span class="item-title" style="text-transform: capitalize;"><?php echo $item; ?></span><a class="sllw-action hide-if-no-js"></a></h5>
						<div class="sllw-edit-item">
							<label for="<?php echo $this->get_field_id('item'.$num); ?>"><?php echo __("Icon:", BASEMENT_TEXTDOMAIN); ?></label>

							<div style="position: relative;margin-right: 25px;padding-top: 6px;">
								<input class="widefat text-coper" id="<?php echo $this->get_field_id('item'.$num); ?>" name="<?php echo $this->get_field_name('item'.$num); ?>" type="text" value="<?php echo $item; ?>" placeholder="<?php _e('e.g.: fa fa-envelope', BASEMENT_TEXTDOMAIN) ?>"/>
