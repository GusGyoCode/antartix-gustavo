...
foreach ( $graph_lines_data as $line ) {
	$unit = ( '' !== $units ) ?  $units : '';
	$id_progress = uniqid('basement_counter-');
	$output .= '<div class="progress-wrapper' . ( ( isset( $line['color'] ) && 'custom' !== $line['color'] ) ?
			' vc_progress-bar-color-' . $line['color'] : '' )
		. '">';
	$output .= '<h6' . $line['txtcolor'] . ' class="clearfix">' . $line['label'] . '<div class="pull-right" ' . $line['txtcolor'] . '><span id="'.$id_progress.'" class="basement_counter" data-to="' . esc_attr( $line['value'] ) . '" data-speed="300" data-refresh-interval="1" data-from="0">' . esc_attr( $line['value'] ) . '</span>'.$unit.'</div></h6>';
	if ( $max_value > 100.00 ) {
	...
	$id = uniqid('vc-progress-id-');
	$style_inne = '';
	if(!empty($line['bgcolor'])) {
		$s_bg = preg_replace('/style="background-color:(.*?);"/','$1',$line['bgcolor']);
		$style_inne = "<style>#{$id}:after {border-color:{$s_bg};}</style>";
	}

	$output .= $style_inne . '<div class="progress"><div id="'.$id.'" class="progress-bar' . esc_attr( implode( ' ', $bar_options ) ) . '" '.$line['bgcolor'].' role="progressbar" aria-valuemin="0" data-end="' . esc_attr( $line['value'] ) . '" aria-valuemax="100" aria-valuenow="' . esc_attr( $line['value'] ) . '"></div></div>';
	$output .= '</div>';
}
