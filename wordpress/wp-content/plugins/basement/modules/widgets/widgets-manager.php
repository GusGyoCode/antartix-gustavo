...
function widgets_scripts($hook) {
	...
	// widget rules JS
	wp_register_script('widget-rules-js', Basement::url() . '/assets/javascript/widget-rules.min.js', array('jquery'));
...
function cherry_dynamic_sidebar_params( $params ) {
	...
	$esc_custom_class = esc_attr($custom_class[$widget_id]);
	$esc_responsive_class = esc_attr($responsive[$widget_id]);
	...
	if ( isset($custom_class[$widget_id]) && !empty($custom_class[$widget_id]) )
		$params[0]['before_widget'] = preg_replace( '/class="/', "class=\"{$esc_custom_class} ", $params[0]['before_widget'], 1 );

	if ( isset($responsive[$widget_id]) && !empty($responsive[$widget_id]) )
	$params[0]['before_widget'] = preg_replace( '/class="/', "class=\"{$esc_responsive_class} ", $params[0]['before_widget'], 1 );
...
function cherry_widget_control() {
	...
	echo '<p class="custom_class"><label for="' . $themename . '_widget_custom_class_'.$id.'">'.__('Custom CSS class', BASEMENT_TEXTDOMAIN).': <input type="text" class="widefat" name="' . $themename . '_widget_custom_class_'.$id.'"  id="' . $themename . '_widget_custom_class_'.$id.'" value="'.$c_class.'" /></label></p>';

	echo '<a class="gk_widget_rules_btn button">'.__('Visibility', BASEMENT_TEXTDOMAIN).'</a><div class="gk_widget_rules_wrapper'.((isset($_COOKIE['gk_last_opened_widget_rules_wrap']) && $_COOKIE['gk_last_opened_widget_rules_wrap'] == 'gk_widget_rules_form_'.$id) ? ' active' : '').'" data-id="gk_widget_rules_form_'.$id.'"><p><label for="' . $themename . '_widget_rules_'.$id.'">'.__('Visible at', BASEMENT_TEXTDOMAIN).': </label><select name="' . $themename . '_widget_rules_type_'.$id.'" id="' . $themename . '_widget_rules_type_'.$id.'" class="widefat gk_widget_rules_select"><option value="all"'.(($value_type != "include" && $value_type != 'exclude') ? " selected=\"selected\"":"").'>'.__('All pages', BASEMENT_TEXTDOMAIN).'</option><option value="exclude"'.(($value_type == "exclude") ? " selected=\"selected\"":"").'>'.__('All pages expecting', BASEMENT_TEXTDOMAIN).':</option><option value="include"'.(($value_type == "include") ? " selected=\"selected\"":"").'>'.__('No pages expecting', BASEMENT_TEXTDOMAIN).':</option></select></p><fieldset class="gk_widget_rules_form" id="gk_widget_rules_form_'.$unique_id.'" data-id="gk_widget_rules_form_'.$id.'"><legend>'.__('Select page to add', BASEMENT_TEXTDOMAIN).'</legend><select class="widefat gk_widget_rules_form_select"><option value="homepage">'.__('Homepage', BASEMENT_TEXTDOMAIN).'</option><option value="blog">'.__('Blog', BASEMENT_TEXTDOMAIN).'</option><option value="page:">'.__('Page', BASEMENT_TEXTDOMAIN).'</option><option value="post:">'.__('Post', BASEMENT_TEXTDOMAIN).'</option><option value="category:">'.__('Archive', BASEMENT_TEXTDOMAIN).'</option><option value="tag:">'.__('Tag', BASEMENT_TEXTDOMAIN).'</option><option value="archive">'.__('Archive', BASEMENT_TEXTDOMAIN).'</option><option value="author:">'.__('Author', BASEMENT_TEXTDOMAIN).'</option><option value="search">'.__('Search page', BASEMENT_TEXTDOMAIN).'</option><option value="page404">'.__('404 page', BASEMENT_TEXTDOMAIN).'</option></select><p><label>'.__('Page ID/Title/slug', BASEMENT_TEXTDOMAIN).':<input type="text" class="gk_widget_rules_form_input_page" /></label></p><p><label>'.__('Post ID/Title/slug', BASEMENT_TEXTDOMAIN).':<input type="text" class="gk_widget_rules_form_input_post" /></label></p><p><label>'.__('Category ID/Name/slug', BASEMENT_TEXTDOMAIN).':<input type="text" class="gk_widget_rules_form_input_category" /></label></p><p><label>'.__('Tag ID/Name', BASEMENT_TEXTDOMAIN).':<input type="text" class="gk_widget_rules_form_input_tag" /></label></p><p><label>'.__('Author', BASEMENT_TEXTDOMAIN).':<input type="text" class="gk_widget_rules_form_input_author" /></label></p><p><button class="gk_widget_rules_btn button-secondary">'.__('Add page', BASEMENT_TEXTDOMAIN).'</button></p><input type="text" name="' . $themename . '_widget_rules_'.$id.'"  id="' . $themename . '_widget_rules_'.$id.'" value="'.$value.'" class="gk_widget_rules_output" /><fieldset class="gk_widget_rules_pages"><legend>'.__('Selected pages', BASEMENT_TEXTDOMAIN).'</legend><span class="gk_widget_rules_nopages">'.__('No pages', BASEMENT_TEXTDOMAIN).'</span><div></div></fieldset></fieldset><script type="text/javascript">gk_widget_control_init(\'#gk_widget_rules_form_'.$unique_id.'\');</script>';
...
function cherry_widget_control_styles_list($widget_name, $id, $value2, $value3) {
	...
	$items = array('<option value="" selected="selected">'.__('None', BASEMENT_TEXTDOMAIN).'</option>');

	// prepare the responsive select
	$items = array(
		'<option value="visible-all-devices"'.((!$value2 || $value2 == 'visible-all-devices') ? ' selected="selected"' : '').'>'.__('All devices', BASEMENT_TEXTDOMAIN).'</option>',
		'<option value="hidden-xs hidden-sm hidden-md"'.(($value2 == 'hidden-xs hidden-sm hidden-md') ? ' selected="selected"' : '').'>'.__('Large devices', BASEMENT_TEXTDOMAIN).'</option>',
		'<option value="hidden-xs hidden-sm"'.(($value2 == 'hidden-xs hidden-sm') ? ' selected="selected"' : '').'>'.__('Large/Medium devices', BASEMENT_TEXTDOMAIN).'</option>',
		'<option value="hidden-xs hidden-md"'.(($value2 == 'hidden-xs hidden-md') ? ' selected="selected"' : '').'>'.__('Large/Small devices', BASEMENT_TEXTDOMAIN).'</option>',
		'<option value="hidden-sm hidden-md"'.(($value2 == 'hidden-sm hidden-md') ? ' selected="selected"' : '').'>'.__('Large/Extra small devices', BASEMENT_TEXTDOMAIN).'</option>',
		'<option value="hidden-xs"'.(($value2 == 'hidden-xs') ? ' selected="selected"' : '').'>'.__('Large/Medium/Small devices', BASEMENT_TEXTDOMAIN).'</option>',
		'<option value="hidden-sm"'.(($value2 == 'hidden-sm') ? ' selected="selected"' : '').'>'.__('Large/Medium/Extra small devices', BASEMENT_TEXTDOMAIN).'</option>',
		'<option value="hidden-md"'.(($value2 == 'hidden-md') ? ' selected="selected"' : '').'>'.__('Large/Small/Extra small devices', BASEMENT_TEXTDOMAIN).'</option>',
		'<option value="hidden-lg"'.(($value2 == 'hidden-lg') ? ' selected="selected"' : '').'>'.__('Medium/Small/Extra small devices', BASEMENT_TEXTDOMAIN).'</option>',
		'<option value="hidden-xs hidden-sm hidden-lg"'.(($value2 == 'hidden-xs hidden-sm hidden-lg') ? ' selected="selected"' : '').'>'.__('Medium devices', BASEMENT_TEXTDOMAIN).'</option>',
		'<option value="hidden-xs hidden-lg"'.(($value2 == 'hidden-xs hidden-lg') ? ' selected="selected"' : '').'>'.__('Medium/Small devices', BASEMENT_TEXTDOMAIN).'</option>',
		'<option value="hidden-sm hidden-lg"'.(($value2 == 'hidden-sm hidden-lg') ? ' selected="selected"' : '').'>'.__('Medium/Extra small devices', BASEMENT_TEXTDOMAIN).'</option>',
		'<option value="hidden-xs hidden-md hidden-lg"'.(($value2 == 'hidden-xs hidden-md hidden-lg') ? ' selected="selected"' : '').'>'.__('Small devices', BASEMENT_TEXTDOMAIN).'</option>',
		'<option value="hidden-md hidden-lg"'.(($value2 == 'hidden-md hidden-lg') ? ' selected="selected"' : '').'>'.__('Small/Extra small devices', BASEMENT_TEXTDOMAIN).'</option>',
		'<option value="hidden-sm hidden-md hidden-lg"'.(($value2 == 'hidden-sm hidden-md hidden-lg') ? ' selected="selected"' : '').'>'.__('Extra small devices', BASEMENT_TEXTDOMAIN).'</option>'

		/*'<option value="hidden-md hidden-lg"'.(($value2 == 'hidden-xs hidden-md hidden-lg') ? ' selected="selected"' : '').'>'.__('Small/Extra small devices', BASEMENT_TEXTDOMAIN).'</option>',
		'<option value="hidden-xs hidden-lg"' . ( ( $value2 == 'hidden-xs hidden-lg' ) ? ' selected="selected"' : '' ) . '>' . __( 'Tablets', BASEMENT_TEXTDOMAIN ) . '</option>',
		'<option value="hidden-sm hidden-md hidden-lg"' . ( ( $value2 == 'hidden-sm hidden-md hidden-lg' ) ? ' selected="selected"' : '' ) . '>' . __( 'Phones', BASEMENT_TEXTDOMAIN ) . '</option>',
		'<option value="hidden-xs"' . ( ( $value2 == 'hidden-xs' ) ? ' selected="selected"' : '' ) . '>' . __( 'Desktops/Tablets', BASEMENT_TEXTDOMAIN ) . '</option>',
		'<option value="hidden-lg"' . ( ( $value2 == 'hidden-lg' ) ? ' selected="selected"' : '' ) . '>' . __( 'Tablets/Phones', BASEMENT_TEXTDOMAIN ) . '</option>',
		'<option value="hidden-sm hidden-md"' . ( ( $value2 == 'hidden-sm hidden-md' ) ? ' selected="selected"' : '' ) . '>' . __( 'Desktops/Phones', BASEMENT_TEXTDOMAIN ) . '</option>'*/
	);
	// output the responsive select
	echo '<p class="device_visible"><label for="' . $themename . '_widget_responsive_'.$id.'">'.__('Visible on', BASEMENT_TEXTDOMAIN).': <select name="' . $themename . '_widget_responsive_'.$id.'"  id="' . $themename . '_widget_responsive_'.$id.'" class="widefat">';
	//
	...
	$items = array(
		'<option value="all"'.(($value3 == null || !$value3 || $value3 == 'all') ? ' selected="selected"' : '').'>'.__('All users', BASEMENT_TEXTDOMAIN).'</option>',
		'<option value="guests"'.(($value3 == 'guests') ? ' selected="selected"' : '').'>'.__('Only guests', BASEMENT_TEXTDOMAIN).'</option>',
		'<option value="registered"'.(($value3 == 'registered') ? ' selected="selected"' : '').'>'.__('Only registered users', BASEMENT_TEXTDOMAIN).'</option>',
		'<option value="administrator"'.(($value3 == 'administrator') ? ' selected="selected"' : '').'>'.__('Only administrator', BASEMENT_TEXTDOMAIN).'</option>'
	);
	//
	echo '<p class="users_visible"><label for="' . $themename . '_widget_users_'.$id.'">'.__('Visible for', BASEMENT_TEXTDOMAIN).': <select name="' . $themename . '_widget_users_'.$id.'"  id="' . $themename . '_widget_users_'.$id.'" class="widefat">';
	//
	foreach($items as $item)
		echo $item;
	//
